...
import { CodeItemDto, PointTypesService } from '../point-types.service';
import { ToastService } from '../ui/toast.service';

@Component({
  selector: 'app-point-types-page',
  standalone: true,
  imports: [CommonModule, FormsModule, TranslatePipe],
  template: `
    <h2>{{ 'codelist.POINT_TYPE' | t }}</h2>
    ...
    <section class="card">
      ...
      <div class="row">
        <label>
          {{ 'table.code' | t }}
          <input [(ngModel)]="draft.code" placeholder="VALVE" />
        </label>
        <label>
          {{ 'table.labelFr' | t }}
...
export class PointTypesPageComponent implements OnDestroy {
  items: CodeItemDto[] = [];
  ...
  constructor(
    ...
    this.sub = this.pointTypes.stream().subscribe((items) => {
      this.items = (items ?? []).map((i) => ({ ...i }));
    });
    this.pointTypes.reload().subscribe();
  }
  ...
  create() {
    ...
    this.api
      .upsertCodeItem('POINT_TYPE', code, {
  ...
  save(item: CodeItemDto) {
    this.api
      .upsertCodeItem('POINT_TYPE', item.code, {
  ...
  remove(item: CodeItemDto) {
    this.api.deleteCodeItem('POINT_TYPE', item.code).subscribe({
      next: () => {
        this.toast.push('success', this.i18n.t('common.saved'));
        this.pointTypes.reload().subscribe();
      },
